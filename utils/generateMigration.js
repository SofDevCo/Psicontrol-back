...
module.exports = {
  up: async (queryInterface, Sequelize) => {
    return queryInterface.createTable('${tableName}', ${JSON.stringify(
    rawAttributes,
    null,
    2
  )});
...
const generateAlterMigration = (
  ...
  Object.keys(newAttributes).forEach((columnName) => {
    if (!existingAttributes[columnName]) {
      alterColumns.push(
        `addColumn('${columnName}', ${JSON.stringify(
...
module.exports = {
  ...
  down: async (queryInterface, Sequelize) => {
    ...
    console.log(
      `Tabela ${tableName} já está sincronizada com o modelo. Nenhuma migração necessária.`
    );
  }
};
...
fs.readdirSync(modelsDir).forEach(async (file) => {
  if (file.endsWith("Model.js")) {
    ...
    if (!model || !model.name) {
      console.error(
        `Erro: O arquivo ${file} não é um model válido ou está mal definido.`
      );
      return;
    }
    ...
    try {
      ...
      console.log(
        `Tabela ${tableName} não encontrada. Criando migração de criação...`
      );
